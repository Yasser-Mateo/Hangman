...
def jugar():
    ...
    while True:
        ...
        if intentos <= 0:
            print("Mala suerte, has perdido :(. La palabra era: ")
            imprimirPalabraOriginal()
            return
        if haGanado():
            print("Has conseguido armar la palabra, has ganado:).")
            return


def ajustes():
    if input("Ingrese su contraseña: ") != palabraSecretaAjustes:
...
1. Jugar
2. Instrucciones
3. Ajustes del banco de palabras
4. Salir del juego
Seleccione: """
    eleccion = int(input(menu))
